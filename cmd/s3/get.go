...
renamed on success, so a failure never leaves a half-written file.
  ...
  s3 get s3://archive/2026/report.pdf ./report.pdf`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := downloadFile(context.Background(), client, b.Bucket, key, args[1]); err != nil {
			return err
		}
...
// downloadFile downloads bucket/key to the local path dst. Large objects
// download in concurrent parts; the data is written to a temp file first,
// then atomically renamed on success, so a failure never leaves a
// half-written file.
func downloadFile(ctx context.Context, client *s3.Client, bucket, key, dst string) error {
	...
	_, err = manager.NewDownloader(client).Download(ctx, tmp, &s3.GetObjectInput{
		Bucket: &bucket,
		Key:    &key,
	})
